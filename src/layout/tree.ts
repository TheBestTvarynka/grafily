...
function buildPreNodes(
    ...
    const parents = getChildNodesIds(perspectiveId, family);
...
function finalizeNodesLayout(
    ...
    const parents = getChildNodesIds(nodeId, family);
    ...
    if (nodeId.type === MARRIAGE_TYPE) {
        ...
        if (marriage.parent1Id) {
            const person = family.personById.get(marriage.parent1Id);
            if (!person) {
        ...
        if (marriage.parent2Id) {
            const person = family.personById.get(marriage.parent2Id);
            if (!person) {
        ...
        const person = family.personById.get(nodeId.id);
        if (!person) {
        ...
        for (const childId of family.personChildren.get(nodeId.id) ?? []) {
            edges.push({
