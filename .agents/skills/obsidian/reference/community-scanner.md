...
As of `eslint-plugin-obsidianmd` **v0.4.0**, the scanner ruleset is published _as_ the plugin's `recommended` config (the "Community scanners ruleset"). A single `...obsidianmd.configs.recommended` reproduces what the scanner runs locally — it bundles:
