...
The Grafily expects that your vault has **one page per person**. But it does not mean that all pages in the vault must be dedicated only to persons.
...
Metadata is a predefined information at the start of each page. Here is a metadata template:
...
Moreover, you do not need to specify the spouse link for both; only one link is sufficient. For example, if you specify in the Bob's metadata that his spouse is Emma, then it is not required to specify Bob in Emma's metadata.

Now you understand the metadata format and its meaning and are ready to follow the [Getting started](./GETTING_STARTED.md) guide to see the plugin in action.
